...
func TestFuturesSnapshotsAndTicks(t *testing.T) {
	c, _ := newClient(t, "/market-data/futures/snapshots/list", "futures_snapshots.json", 0)
	...
	c2, _ := newClient(t, "/market-data/futures/ticks/list", "futures_ticks.json", 0)
	ticks, err := c2.FuturesTicks(context.Background(), "MESmain", 5)
	if err != nil {
		t.Fatal(err)
	}
	if ticks.InstrumentID != "470005101" || len(ticks.Ticks) == 0 || ticks.Ticks[0].Side != TickBuy {
...
func TestCryptoSnapshotsAndBars(t *testing.T) {
	c, _ := newClient(t, "/market-data/crypto/snapshots/list", "crypto_snapshots.json", 0)
	snaps, err := c.CryptoSnapshots(context.Background(), []string{"BTCUSD", "ETHUSD"})
	if err != nil {
		t.Fatal(err)
	}
	if snaps[0].Symbol != "BTCUSD" || !snaps[0].BidSize.Decimal.Equal(d("0.000022")) {
		t.Errorf("snapshot = %+v", snaps[0])
	}
...
func TestEventSnapshotDepthTicks(t *testing.T) {
	c, _ := newClient(t, "/market-data/event-contracts/snapshots/list", "event_snapshots.json", 0)
	...
	tk := ticks.Ticks[0]
	if tk.Side != No || tk.TradeID == "" || !tk.YesPrice.Equal(d("0.001")) || !tk.NoPrice.Equal(d("0.999")) {
		t.Errorf("tick = %+v", tk)
	}
}
